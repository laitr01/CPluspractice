//
// Created by ltrach on 8/23/2018.
//
...
extern "C" {
jstring Java_com_bestapplication_cpluspractice_MainActivity_naGetTitle(
        JNIEnv *env,
        jobject /* this */) {
...
void Java_com_bestapplication_cpluspractice_MainActivity_naToGrayscale(
        JNIEnv *env,
        jclass clazz,
        jobject srcBitmap
) {
    ...
    if ((srcResult = AndroidBitmap_getInfo(env, srcBitmap, &srcInfo)) < 0) {
    ...
    if ((srcResult = AndroidBitmap_lockPixels(env, srcBitmap, &srcPixels)) < 0) {
    ...
    for (h = 0; h < srcInfo.height; h++) {
        ...
        for (w = 0; w < srcInfo.width; w++) {
            red = (line[w] & 0x00ff0000) >> 16;
            green = (line[w] & 0x0000ff00) >> 8;
            blue = (line[w] & 0x000000ff);
...
void Java_com_bestapplication_cpluspractice_MainActivity_naToBlackWhite(
        JNIEnv *env,
        jclass clazz,
        jobject srcBitmap
) {
...
void Java_com_bestapplication_cpluspractice_MainActivity_naSetBrightness(
        JNIEnv *env,
        jclass clazz,
        jobject pSrcBitmap,
...
void Java_com_bestapplication_cpluspractice_MainActivity_naSetGaussianBlur(
        JNIEnv *env,
        jclass clazz,
        jobject pSrcBitmap,
        jobject pOutBitmap
    ...
    LOGDE("Retrieved information of bitmap");
